...
{
	if (!settings || !source_) {
		throw std::invalid_argument("Invalid Linux background-removal context arguments");
	}
	...
	backgroundRemovalEffect_ = std::make_unique<Effects::BackgroundRemovalEffect>();
}
...
auto FilterContext::create(obs_data_t *settings, obs_source_t *source) noexcept -> void *
{
	try {
		...
		blog(LOG_ERROR, OBS_LOG_HEADER "Failed to create Linux FilterContext: %s", exception.what());
	} catch (...) {
		blog(LOG_ERROR, OBS_LOG_HEADER "Failed to create Linux FilterContext");
	}
	return nullptr;
}
...
{
	obs_data_set_default_int(settings, "v2_Model_PipelineId", MediaPipeLandscapeNcnnPipeline::kPipelineId);
...
auto FilterContext::getProperties(void *) noexcept -> obs_properties_t *
{
	...
	obs_property_list_add_int(model, obs_module_text("MediaPipe (Landscape) - ncnn"),
				  MediaPipeLandscapeNcnnPipeline::kPipelineId);
	obs_property_list_add_int(model, obs_module_text("MediaPipe (Landscape) - ncnn (CPU)"),
				  MediaPipeLandscapeNcnnCpuForcedPipeline::kPipelineId);
...
void FilterContext::update(void *data, obs_data_t *settings) noexcept
{
	...
	blog(LOG_INFO,
	     OBS_LOG_HEADER
	     "Updating Linux settings: pipeline_id=%lld sensitivity=%.3f refine=%d threshold=%.3f contour=%.3f expansion=%lld blur=%d blur_factor=%lld focus_point=%.3f focus_depth=%.3f",
...
void FilterContext::videoTick(void *data, float seconds) noexcept
{
	...
	if (requiresPipelineRecreation) {
		...
		try {
			...
			blog(LOG_ERROR, OBS_LOG_HEADER "Failed to recreate ncnn rendering pipeline: %s",
			     exception.what());
		}
		if (pipeline) {
