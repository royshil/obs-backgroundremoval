...
{
	if (property.pipelineId == MediaPipeLandscapeCoreMLPipeline::kPipelineId) {
		blog(LOG_INFO, OBS_LOG_HEADER "Recreating CoreML rendering pipeline (%ux%u)", width, height);
		return std::make_shared<MediaPipeLandscapeCoreMLPipeline>(property, source, backgroundRemovalEffect,
									  width, height);
	}
