...
#include "EmbeddedModels/mediapipe_selfie_segmentation_landscape_fp16_ncnn.id.h"
...
MediaPipeLandscapeNcnnPipeline::MediaPipeLandscapeNcnnPipeline(
	const FilterProperty &property, obs_source_t *source, Effects::BackgroundRemovalEffect &backgroundRemovalEffect,
	std::unique_ptr<ncnn::Net> network, std::uint32_t width, std::uint32_t height)
	  ...
	  outputMorphology_({width, height})
{
	...
	if (!source || !network_ || width == 0 || height == 0) {
		throw std::invalid_argument("Invalid MediaPipe Landscape ncnn pipeline arguments");
	}
...
MediaPipeLandscapeNcnnPipeline::~MediaPipeLandscapeNcnnPipeline() noexcept
{
...
auto MediaPipeLandscapeNcnnPipeline::processImage(const std::uint8_t *inputData, std::uint32_t linesize,
						  std::uint8_t *output) noexcept -> bool
{
	if (!inputData || !output || linesize < modelWidth * sizeof(float)) {
		return false;
	}
	ncnn::Mat input(static_cast<int>(modelWidth), static_cast<int>(modelHeight), 3, sizeof(float));
	for (std::uint32_t channel = 0; channel < 3; ++channel) {
		float *destination = input.channel(static_cast<int>(channel));
		for (std::uint32_t row = 0; row < modelHeight; ++row) {
			...
			std::memcpy(destination + static_cast<std::size_t>(row) * modelWidth, source,
				    modelWidth * sizeof(float));
		}
	}

	ncnn::Extractor extractor = network_->create_extractor();
	ncnn::Mat result;
	const bool inputAccepted = extractor.input(model_fp16_ncnn_param_id::BLOB_in0, input) == 0;
	const bool outputExtracted = inputAccepted &&
				     extractor.extract(model_fp16_ncnn_param_id::BLOB_out0, result) == 0;
	const bool outputLayoutValid = outputExtracted && !result.empty() && result.w == static_cast<int>(modelWidth) &&
				       result.h == static_cast<int>(modelHeight) && result.c == 1;
	if (!outputLayoutValid) {
		return false;
	}
	const float *resultData = result.channel(0);
	for (std::size_t index = 0; index < modelPixelCount; ++index) {
		output[index] =
			static_cast<std::uint8_t>(std::clamp((1.0F - resultData[index]) * 255.0F, 0.0F, 255.0F));
	}
	return true;
}

void MediaPipeLandscapeNcnnPipeline::update(const FilterProperty &property) noexcept
{
...
auto MediaPipeLandscapeNcnnPipeline::shouldInferMask(const ObsBridgeUtils::TextureReader *textureReader,
...
auto MediaPipeLandscapeNcnnPipeline::inferMask(const ObsBridgeUtils::TextureReader *textureReader) noexcept -> bool
{
	const auto &buffer = textureReader->getBuffer();
	const std::uint32_t linesize = textureReader->getBufferLinesize();
	...
	if (!inferenceSucceeded) {
		blog(LOG_ERROR, OBS_LOG_HEADER "MediaPipe Landscape ncnn inference failed");
		hasLastInput_ = false;
		return false;
	}
	hasLastInput_ = true;
...
auto MediaPipeLandscapeNcnnPipeline::refinementFailed(const char *message) noexcept -> bool
{
...
auto MediaPipeLandscapeNcnnPipeline::refineMask(bool refineMaskEnabled, float refineMaskThreshold,
...
void MediaPipeLandscapeNcnnPipeline::videoTick(float) noexcept {}

auto MediaPipeLandscapeNcnnPipeline::blurBackground(gs_texture_t *sourceTexture, gs_texture_t *alphaTexture,
						    const std::array<gs_texture_t *, 2> &blurTextures,
						    std::size_t iterations, float focusPoint, float focusDepth) noexcept
	-> gs_texture_t *
{
...
void MediaPipeLandscapeNcnnPipeline::videoRender(gs_effect_t *) noexcept
{
